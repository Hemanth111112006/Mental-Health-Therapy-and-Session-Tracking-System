...
export function AuthProvider({ children }) {
  ...
  // ── Login with OAuth (Google, Microsoft, Apple - Free & Seamless) ───────
  const loginWithOAuth = useCallback(async (oauthData) => {
    ...
    try {
      ...
      const resolvedFirstName = responseData.firstName || decoded?.firstName || 'Hemanth';
      const resolvedLastName = responseData.lastName || decoded?.lastName || 'K';
      ...
      const safeUser = {
        ...
        lastName: resolvedLastName,
        title: `${resolvedFirstName} ${resolvedLastName}`.trim(),
        avatar: (resolvedFirstName[0] || 'H').toUpperCase(),
      };
