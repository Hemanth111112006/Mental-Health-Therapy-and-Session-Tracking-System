...
const PsychologistAppointmentsPage = () => {
  ...
  const handleNewSession = async () => {
    ...
    if (!time) return;
    try {
      await appointmentApi.scheduleAppointment({ date, startTime: time, endTime: time, type: 'ASSESSMENT', telehealth: false, clientId: Number(clientId) });
  ...
  const handleRebook = async (appt) => {
    ...
    try {
      await appointmentApi.scheduleAppointment({
        ...
        startTime: time,
        endTime: time,
        type: appt.type || appt.appointmentType || 'ASSESSMENT',
    ...
    <div className="mc-page-container" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontSize: '24px', fontWeight: 700, color: 'var(--text-primary)' }}>Psychologist Appointments</h1>
            <p style={{ margin: '4px 0 0 0', fontSize: '13px', color: 'var(--text-secondary)' }}>Manage your assessment sessions. ({appointments.length} total)</p>
          </div>
        </div>
      ...
      <div style={{ padding: '16px', backgroundColor: 'var(--bg-secondary)', borderRadius: '12px', border: '1px solid var(--border-primary)', display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ flex: '1 1 280px', position: 'relative' }}>
          ...
          <input type="text" placeholder="Search clients or assessment types..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                        ...
                        {isCheckedIn && (
                          <>
                            <button 
                              ...
                              title="Start testing battery or outcome measures"
