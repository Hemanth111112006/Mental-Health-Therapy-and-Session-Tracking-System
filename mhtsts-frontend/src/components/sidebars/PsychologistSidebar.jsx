...
import {
  BrainCircuit, UsersRound, CalendarCheck, CalendarSearch, TrendingUp,
  ClipboardCheck, BookOpenText, MessagesSquare, BarChart3, Brain,
  Sliders, DoorOpen
} from 'lucide-react';

// Icon substitutions made:
// - ChartSpline → TrendingUp (ChartSpline does not exist in lucide-react)
// - FileChartColumn → BarChart3 (FileChartColumn does not exist in lucide-react)
// - CalendarFold → CalendarSearch (CalendarFold does not exist in lucide-react)

const PsychologistSidebar = ({ collapsed }) => {
  ...
  const menuItems = [
    { to: '/psychologist/dashboard', icon: <BrainCircuit size={18} />, label: 'Dashboard' },
    { to: '/clients', icon: <UsersRound size={18} />, label: 'Clients' },
    { to: '/psychologist/appointments', icon: <CalendarCheck size={18} />, label: 'Appointments' },
    { to: '/psychologist/calendar', icon: <CalendarSearch size={18} />, label: 'Calendar' },
    { to: '/outcome-measures', icon: <TrendingUp size={18} />, label: 'Psychological Assessments' },
    { to: '/treatment-plans', icon: <ClipboardCheck size={18} />, label: 'Therapy Plans' },
    { to: '/session-notes', icon: <BookOpenText size={18} />, label: 'Session Notes' },
  ...
  return (
    <aside className={`mc-sidebar ${collapsed ? 'collapsed' : ''}`}>
      <div className="mc-sidebar-brand" style={{ padding: '12px 20px', borderBottom: '1px solid var(--border-primary)' }}>
        ...
        {!collapsed && (
          <div>
            <div className="mc-sidebar-title" style={{ fontSize: 14, fontWeight: 800 }}>MindCare Psychology</div>
