...
const registerPageContent = `import { useState } from 'react';
        ...
        return (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
              <div style={{ marginBottom: 16 }}>
                ...
                <div className="mc-luxury-input-wrapper">
                  <PersonOutlinedIcon style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-placeholder)', fontSize: 18 }} />
                  <input className={\`mc-luxury-input \${errors.firstName ? 'error' : ''}\`} style={{ paddingLeft: 40 }} placeholder="First name" value={formData.firstName} onChange={e => updateField('firstName', e.target.value)} />
                </div>
                {errors.firstName && <div style={{ fontSize: 10, color: "var(--color-danger)", marginTop: 2 }}>{errors.firstName}</div>}
              </div>
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#344054", marginBottom: 4 }}>Last Name <span className="required">*</span></label>
                <input className={\`mc-luxury-input \${errors.lastName ? 'error' : ''}\`} placeholder="Last name" value={formData.lastName} onChange={e => updateField('lastName', e.target.value)} />
                {errors.lastName && <div style={{ fontSize: 10, color: "var(--color-danger)", marginTop: 2 }}>{errors.lastName}</div>}
              </div>
            </div>
            <div style={{ marginBottom: 16 }}>
              ...
              <div className="mc-luxury-input-wrapper">
                <EmailOutlinedIcon style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-placeholder)', fontSize: 18 }} />
                <input className={\`mc-luxury-input \${errors.email ? 'error' : ''}\`} style={{ paddingLeft: 40 }} type="email" placeholder="you@example.com" value={formData.email} onChange={e => updateField('email', e.target.value)} />
              </div>
              {errors.email && <div style={{ fontSize: 10, color: "var(--color-danger)", marginTop: 2 }}>{errors.email}</div>}
            </div>
            <div style={{ marginBottom: 16 }}>
              ...
              <div className="mc-luxury-input-wrapper">
                <PhoneOutlinedIcon style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-placeholder)', fontSize: 18 }} />
                <input className={\`mc-luxury-input \${errors.phone ? 'error' : ''}\`} style={{ paddingLeft: 40 }} placeholder="(555) 123-4567" value={formData.phone} onChange={e => updateField('phone', e.target.value)} />
              </div>
              {errors.phone && <div style={{ fontSize: 10, color: "var(--color-danger)", marginTop: 2 }}>{errors.phone}</div>}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#344054", marginBottom: 4 }}>Date of Birth</label>
                <input className="mc-luxury-input" type="date" value={formData.dateOfBirth} onChange={e => updateField('dateOfBirth', e.target.value)} />
              </div>
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#344054", marginBottom: 4 }}>Gender</label>
                <select className="mc-luxury-input" value={formData.gender} onChange={e => updateField('gender', e.target.value)}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div className="mc-luxury-input-wrapper">
                <LockOutlinedIcon style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-placeholder)', fontSize: 18 }} />
                <input className={\`mc-luxury-input \${errors.password ? 'error' : ''}\`} style={{ paddingLeft: 40, paddingRight: 40 }} type={showPassword ? 'text' : 'password'} placeholder="Min 8 characters" value={formData.password} onChange={e => updateField('password', e.target.value)} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-placeholder)', padding: 4, display: 'flex' }}>
                  {showPassword ? <VisibilityOffOutlinedIcon style={{ fontSize: 18 }} /> : <VisibilityOutlinedIcon style={{ fontSize: 18 }} />}
                </button>
              </div>
              {errors.password && <div style={{ fontSize: 10, color: "var(--color-danger)", marginTop: 2 }}>{errors.password}</div>}
              <div className="mc-form-helper">Must include uppercase, lowercase, number, and special character</div>
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#344054", marginBottom: 4 }}>Confirm Password <span className="required">*</span></label>
              <input className={\`mc-luxury-input \${errors.confirmPassword ? 'error' : ''}\`} type="password" placeholder="Re-enter password" value={formData.confirmPassword} onChange={e => updateField('confirmPassword', e.target.value)} />
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
              <div style={{ marginBottom: 16 }}>
                ...
                <div className="mc-luxury-input-wrapper">
                  <BadgeOutlinedIcon style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-placeholder)', fontSize: 18 }} />
                  <input className={\`mc-luxury-input \${errors.licenseNumber ? 'error' : ''}\`} style={{ paddingLeft: 40 }} placeholder="e.g., LCSW-12345" value={formData.licenseNumber} onChange={e => updateField('licenseNumber', e.target.value)} />
                </div>
                {errors.licenseNumber && <div style={{ fontSize: 10, color: "var(--color-danger)", marginTop: 2 }}>{errors.licenseNumber}</div>}
              </div>
              <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#344054", marginBottom: 4 }}>Supervisor Name (if applicable)</label>
              <input className="mc-luxury-input" placeholder="Supervisor's full name" value={formData.supervisorName} onChange={e => updateField('supervisorName', e.target.value)} />
            </div>
          </div>
        ) : (
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#344054", marginBottom: 4 }}>Emergency Contact Name <span className="required">*</span></label>
              <input className={\`mc-luxury-input \${errors.emergencyContactName ? 'error' : ''}\`} placeholder="Full name" value={formData.emergencyContactName} onChange={e => updateField('emergencyContactName', e.target.value)} />
              {errors.emergencyContactName && <div style={{ fontSize: 10, color: "var(--color-danger)", marginTop: 2 }}>{errors.emergencyContactName}</div>}
            </div>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#344054", marginBottom: 4 }}>Emergency Contact Phone <span className="required">*</span></label>
              <input className={\`mc-luxury-input \${errors.emergencyContactPhone ? 'error' : ''}\`} placeholder="(555) 123-4567" value={formData.emergencyContactPhone} onChange={e => updateField('emergencyContactPhone', e.target.value)} />
              {errors.emergencyContactPhone && <div style={{ fontSize: 10, color: "var(--color-danger)", marginTop: 2 }}>{errors.emergencyContactPhone}</div>}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#344054", marginBottom: 4 }}>Insurance Provider</label>
                <select className="mc-luxury-input" value={formData.insuranceProvider} onChange={e => updateField('insuranceProvider', e.target.value)}>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "#344054", marginBottom: 4 }}>Insurance Member ID</label>
                <input className="mc-luxury-input" placeholder="Member ID" value={formData.insuranceMemberId} onChange={e => updateField('insuranceMemberId', e.target.value)} />
              </div>
            </div>
            <div style={{ marginBottom: 16 }}>
...
fs.writeFileSync('src/features/authentication/pages/RegisterPage.jsx', registerPageContent);
console.log('RegisterPage.jsx completely recreated and fixed.');
